...
进行设计原则量化判断时，先读取 `references/design-decay-metrics.md`。按指标找出候选腐化点，再用源码和工具证据核实。
...
报告面向修复决策，不面向工具调试。先写人能读懂的结论、原则归类和修正路径，再把工具证据放到问题卡片底部或证据附录。

必须包含：

1. `Decision`：`PASS`、`PASS_WITH_WARNINGS`、`REQUIRES_ATTENTION`、`BLOCKED_TOOLING`
2. `一句话结论`：用业务/架构语言说明当前最大问题。
3. `最需要先处理的 3 件事`：按修复优先级给出行动项。
4. `原则归类`：按模块边界、单一职责、依赖倒置、开闭原则、接口隔离、封装度等原则归类。
5. `问题卡片`：每张卡片说明问题、违反原则、为什么有风险、建议修正、验收标准、证据。
6. `下一步建议`：按固定规则选择一个闭环流程或模块继续追踪。
7. `Evidence Appendix`：工具覆盖、模块 YAML、依赖 YAML、复杂度和模式匹配等细节。
...
现象：
用一两句话描述代码现在怎么组织，不要先列工具指标。

为什么这是问题：
说明它会让哪类业务变化、测试、部署或协作变困难。

建议修正：
给出第一步可执行修正，不要求一次性重构完成。
...
- 每个原则先给 `状态`：`正常`、`轻微偏离`、`明显偏离`、`严重偏离`。
- 每个原则只列最关键的 1-3 个问题。
- 每个问题必须指向后续问题卡片编号。
- 工具名和原始指标不要出现在原则归类第一句话里。

修复优先级要求：

- `P0`：真实正确性、安全或运行阻断。
- `P1`：核心业务流程的高复杂度、高耦合、错误依赖方向。
- `P2`：范围较大的治理债务或分阶段拆分项。
- `P3`：清理类问题。

不要让报告以工具发现开头。工具证据是支撑材料，不是报告主体。

审计记录文件要求：

- 每次完整审计必须同时输出到对话和文件。
- 文件写入目标仓库的 `architecture/audits/` 目录。
- 文件名使用 `architecture-audit-YYYYMMDD-HHMMSS.md`。
- 文件内容必须与最终对话报告一致；如果对话中压缩了内容，文件中保留完整版本。
- 最终回复必须给出报告文件路径。
- 如果目标仓库没有 `architecture/audits/`，创建该目录。
...
next_investigation:
  ...
  why_this_target:
    - "<选择它的原因，不写泛泛原则>"
  trace_to_perform:
    - "<需要追踪的 inbound/outbound 调用或依赖边>"
  boundary_question: "<这段逻辑到底应该归属哪个模块或契约>"
  candidate_destination: "<候选落点，例如 application service、domain service、adapter、contract>"
  first_contract_to_add:
    - "<建议先固化的 import-linter 或模块边界契约>"
  stop_condition:
    - "<达到什么结构变化后本轮追踪结束>"
```

如果没有合适的闭环流程，选择最高优先级的问题卡片，输出最小可验证的下一步核实动作。
